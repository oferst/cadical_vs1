...
namespace CaDiCaL {
	...
	// ofer: the sorting below may be confusing because it is in reverse order, i.e., the clauses we want to vivify first will be *last* in the order.  
	struct vivify_less_clause {
		...
		bool operator () (Clause * a, Clause * b) const {			
			...
			for (i = a->begin(), j = b->begin(); i != eoa && j != eob; i++, j++)
				if (*i != *j) return vivify_more_noccs(internal) (*j, *i); // ofer: the normal exit point. 'true' if 'i' (the literal of a) has *less* occurrences (note the parameter order: (*j, *i)). 

			return i == eoa; // ofer: true if a is shorter. We get here only if a subsumes b.
		}
	};
